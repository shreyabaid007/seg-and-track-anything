...
def tracking_objects(Seg_Tracker, input_video, input_img_seq, fps, enable_redaction=False, redact_idx=None, frame_num=0):
    print("Start tracking!")
    return tracking_objects_in_video(
        Seg_Tracker, input_video, input_img_seq, fps, frame_num,
        enable_redaction=enable_redaction, redact_idx=redact_idx
    )


def apply_redaction(frame, mask, redact_idx=None):
    """Apply black redaction to masked areas"""
    if redact_idx is not None:
        # Redact specific object index
        redaction_mask = mask != redact_idx
    else:
        # Redact all non-zero indices
        redaction_mask = mask > 0

    frame_copy = frame.copy()
    frame_copy[redaction_mask] = [0, 0, 0]
    return frame_copy


def draw_redacted_mask(frame, mask, redact_idx=None):
    """Wrapper to support both redaction and original mask visualization"""
    if redact_idx is not None:
        return apply_redaction(frame, mask, redact_idx)
    ...
    with app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=0.5):
                enable_redaction = gr.Checkbox(label="Enable Redaction", value=True)
                redact_idx = gr.Number(label="Object Index to Redact (empty for all)", value=None)
