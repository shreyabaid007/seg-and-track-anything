...
def tracking_objects_in_video(SegTracker, input_video, input_img_seq, fps, frame_num=0, enable_redaction=False,
                              redact_idx=None):
...
def video_type_input_tracking(SegTracker, input_video, io_args, video_name, frame_num=0):
    ...
    while cap.isOpened():
        ...
        masked_frame = draw_mask(frame, pred_mask,
                                 enable_redaction=io_args['enable_redaction'],
                                 redact_idx=io_args['redact_idx'])
...
def draw_mask(img, mask, alpha=0.5, id_countour=False, enable_redaction=False, redact_idx=None):
    if enable_redaction:
        img_copy = img.copy()

        if redact_idx is not None:
            # Redact specific index
            mask_to_redact = (mask != redact_idx) & (mask > 0)
        else:
            # Redact all non-zero indices
            mask_to_redact = mask > 0

        # Get masked region coordinates
        y_indices, x_indices = np.where(mask_to_redact)
        if len(y_indices) > 0 and len(x_indices) > 0:
            # Get bounds of the masked region
            y_min, y_max = np.min(y_indices), np.max(y_indices)
            x_min, x_max = np.min(x_indices), np.max(x_indices)

            # Extract region to be pixelated
            region = img_copy[y_min:y_max + 1, x_min:x_max + 1]

            # Calculate pixel size based on face size with finer pixelation
            height, width = region.shape[:2]
            face_size = min(height, width)

            # Smaller pixelation: use 15-20 pixels across face instead of 8-12 | 20 4
            pixel_size = max(face_size // 10, 6)  # Increased division factor for smaller pixels

            # Reduced minimum pixel size for finer detail 6
            pixel_size = max(pixel_size, 4)  # Reduced from 12 to 6

            pixel_size = max(pixel_size, 6) #todo:

            # Calculate dimensions ensuring we don't get too small
            h = pixel_size if height > pixel_size else height
            w = pixel_size if width > pixel_size else width

            # Multi-step pixelation for smoother effect
            # First downscale more gradually
            temp_h = max(height // pixel_size * 2, 1)  # Double the intermediate size
            temp_w = max(width // pixel_size * 2, 1)  # Double the intermediate size

            # Three-step pixelation for smoother results
            pixelated = cv2.resize(region, (temp_w, temp_h), interpolation=cv2.INTER_LINEAR)
            pixelated = cv2.resize(pixelated, (temp_w // 2, temp_h // 2), interpolation=cv2.INTER_LINEAR)
            pixelated = cv2.resize(pixelated, (width, height), interpolation=cv2.INTER_LINEAR)

            # Very light blur to smooth edges while maintaining pixelation
            pixelated = cv2.GaussianBlur(pixelated, (3, 3), 0.5)  # Reduced blur strength

            # Create a mask for the region
            region_mask = mask_to_redact[y_min:y_max + 1, x_min:x_max + 1]

            # Apply pixelated effect only to masked pixels
            region[region_mask] = pixelated[region_mask]
            img_copy[y_min:y_max + 1, x_min:x_max + 1] = region

        return img_copy

    # Original visualization logic remains same
    img_mask = img.copy()
    if id_countour:
        obj_ids = np.unique(mask)
        obj_ids = obj_ids[obj_ids != 0]
